...
const [ip, setip] = useState(localStorage.getItem("localIp"));
  useEffect(() => {
    const fetchData = async () => {
      try {
        const htResponse = await axios.get("http://147.79.68.117:8000/api/getplant");
  ...
  const filteredLedgerData = htLedger.filter(
    (item) => item.mo_year === selectedMonth && item.weg_htscno === conHtScNo
  );
  ...
  const newFilteredData = searchFilteredData.map((row) => {
    return {
      ServiceNumber: row.con_htscno,
      C1: row.G_ALL_C1,
      C2: row.G_ALL_C2,
      C3: 0,
      C4: row.G_ALL_C4,
  ...
  const exportToExcel = () => {
    ...
    const ws = XLSX.utils.json_to_sheet(newFilteredData); // Convert JSON to sheet
    const wb = XLSX.utils.book_new(); // Create a new workbook
    XLSX.utils.book_append_sheet(wb, ws, `Allotment for - ${conHtScNo}`);

    const excelBuffer = XLSX.write(wb, { bookType: "xlsx", type: "array" });
    const data = new Blob([excelBuffer], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });

    // Get local system date and time in 12-hour format with AM/PM
    const now = new Date();
    ...
    const fileName = `Allotment_for_${conHtScNo}_${formattedDate}.xlsx`;

    saveAs(data, fileName); // Save with local system time in 12-hour format
};
  ...
  return (
    <div className="p-6 bg-gray-100 uppercase">
      <div className="text-center font-montserrat font-bold p-6">
        <h1>HT - COM/CON ALLOTMENT DETAIL FROM GEN PLANT</h1>
      </div>
      ...
      <div className="mb-4 flex items-center gap-4">
        ...
        <label>Select GEN PLANT HTSCNo:</label>
        <select
          value={conHtScNo}
          onChange={(e) => setConHtScNo(e.target.value)}
          className="p-2 border rounded-md"
        >
          <option value="" disabled>PLANT HTSCNo</option>
{[...new Set(rows.map(row => row.weg_htscno))].map((htscno, index) => (
  <option key={index} value={htscno}>{htscno}</option>
))}
  ...
  <thead>
    <tr className="bg-gray-200">
      {[
        "id", "con_htscno", "inj_volt", "line_loss",
        "ALL C1", "ALL C2", "ALL C4", "ALL C5","ALL TOTAL",
        "amr", "om", "trans", "soc",
        "rkvah", "iec", "sch", "other", "dsm","CHARGES TOTAL"
      ].map((field) => (
  ...
  <tbody>
    {searchFilteredData.length > 0 ? (
      searchFilteredData.map((row, index) => {
        ...
        const cHRGSTOTAL = [
          row.amr, row.om, row.trans, row.soc,row.rkvah, row.iec, row.sch, row.other, row.dsm
        ].reduce((sum, val) => sum + (Number(val) || 0), 0).toFixed(0);
        ...
        return (
          <tr key={row.id || index} className="border">
            ...
            {[
              "amr", "om", "trans", "soc","rkvah", "iec", "sch", "other", "dsm"            
            ].map((field) => (
              <td key={field} className="border p-2">{formatIndianNumber(row[field]) ?? 0}</td>
            ))}
            <td className="border p-2 font-bold">{formatIndianNumber(cHRGSTOTAL)}</td>
          </tr>
        );
      })
    ) : (
  ...
  {formatIndianNumber(
    ...
    Number(totalValues["G_ALL_C5"] || 0)
      ...
      <td className="border p-2">
        {formatIndianNumber(
          Number(totalValues["amr"] || 0) +
          Number(totalValues["om"] || 0) +
          Number(totalValues["trans"] || 0) +
          Number(totalValues["soc"] || 0)+        
          Number(totalValues["rkvah"] || 0) +
          Number(totalValues["iec"] || 0) +
          Number(totalValues["sch"] || 0) +
          Number(totalValues["other"] || 0) +
          Number(totalValues["dsm"] || 0)
        )}
      </td>
    </tr>
  </tfoot>
