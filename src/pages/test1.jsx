...
const GenBulk = () => {
    ...
    useEffect(() => {
        ...
        const filteredData1 = excelData.filter(plant =>
            ...
            !plantData.some(p => p.weg_htscno === plant.GENPLANTHTSCNO)
          );
          
        console.log(filteredData1);
        
        setProcessingData(filteredData1);
        // Step 1: Add a new field "priority" to each row
        const updatedData = filteredData1.map(row => {
           // Ensure values are strings, remove extra spaces
            const ict = row["IEX/CAPTIVE/THIRD PARTY"].toString().trim();
            const type = row["WIND/SOLAR/BIOMASS/BAGASSE/THERMAL"].toString().trim();
            const rec = row["REC/NON-REC"].toString().trim();

            // Combine only non-empty values (removes extra spaces)
            const matchString = [ict, type, rec].filter(Boolean).join(" ").trim();

            // Debugging - Check what matchString looks like
            // console.log("Match String:", `"${matchString}"`);

            // Step 2: Check if `matchString` exists in `pri.plant_name`
            const pri1 = pri.filter(plant => {
                const plantName = plant.plant_name ? plant.plant_name.trim() : "";
                // console.log("Checking against PRI Name:", `"${plantName}"`);
                return plantName === matchString;
            });

            // console.log("Match PRI:", pri1);


            const filteredData = htDetail.find(plant => plant.htscno === conhtscno);
            ...
            return {
                ...row,
                priority: pri1.length > 0 ? pri1[0].priority : "No Match", // Show "No Match" if nothing found
        ...
        // Step 3: Filter the updated data based on HTSCNO
        const filteredData = updatedData.filter(plant => plant.HTSCNO === conhtscno);

        // Step 4: Set state
        setProcessedData(filteredData);

    }, [excelData, conhtscno, pri]); // Added `pri` as a dependency
                                ...
                                <tbody>
                                    {processedData.map((row, index) => (
                                        <tr key={row.id}>
                                            ...
                                            <td className="border border-gray-300 px-4 py-2">
                                                {`${row["IEX/CAPTIVE/THIRD PARTY"]} ${row["WIND/SOLAR/BIOMASS/BAGASSE/THERMAL"]} ${row["REC/NON-REC"]}`}
                                            </td>
